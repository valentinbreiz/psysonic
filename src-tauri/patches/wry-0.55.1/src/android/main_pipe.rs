...
// PSYSONIC PATCH (mobile PoC): unbounded instead of bounded(8).
//
// The main pipe is drained on the Android UI thread. When the page-load
// callback (`onPageLoaded`, also on the UI thread) blocks on a tauri state
// mutex while the tauri event-loop thread is blocked *sending* the 9th
// pending eval into this channel, the two threads deadlock and Android
// reports an ANR. An unbounded channel removes the send-side block, which
// breaks every cycle through this edge. Revisit when upstream fixes the
// page-load/eval deadlock.
static CHANNEL: Lazy<(
